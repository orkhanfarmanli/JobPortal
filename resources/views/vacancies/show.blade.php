						...
						<div class="row">
							<div class="card-i-header">
								<div class="card-i-header-link">
									<a href="#">{{$vac->subcategories->categories['cat_name']}}</a> /
									<a href="#">{{$vac->subcategories['subcat_name']}}</a> 
								</div>
								
								<div class="row">
									<div class="card-i-header-post">
									<div class="div-i-header-post-title">
										<h2>{{$vac->vac_position}}</h2>
										<span>{{$vac->vac_min_salary}} - {{$vac->vac_max_salary}}   AZN</span>
										<span class="cvName">{{$vac->vac_company_name}}</span>
									</div>

									<div class="div-i-header-post-baxis">
										<span>Baxışların sayı: 1318</span><br>
										<span>Elan #{{$vac->id}}</span>
							  ...
							  <div class="info">
								 <div class="col-md-12">
									<div class="col-md-6">
										...
										<div class="col-md-6">
											<ul>
												<li>{{$vac->city['city_name']}}</li>
												<li>{{$vac->vac_min_age}} - {{$vac->vac_max_age}} yaş</li>
												<li>{{$vac->education['edu_name']}}</li>
												<li>{{$vac->experience['exp_name']}}</li>
												<li>{{$vac->created_at}}</li> 
												<li>{{$vac->updated_at}}</li>
												<li>{{$vac->vac_contact}}</li>
											</ul>
										</div>
									</div>
									<div class="col-md-6">
										<div class="row">
											<div class="col-md-12">
												...
												<div class="col-md-9">
													<span><a href="tel:{{$vac->vac_phone}}">{{$vac->vac_phone}}</a></span>
													<span><a href="tel:{{$vac->vac_phone}}">{{$vac->vac_phone}}</a></span>
													<span><a href="tel:{{$vac->vac_phone}}">{{$vac->vac_phone}}</a></span>
												</div>
											</div>		
										...
										<div class="row">
											<div class="col-md-12">
												...
												<div class="col-md-9"><span class="email"><a encode="javascript" href="mailto:{{$vac->vac_email}}">{{$vac->vac_email}}</a></span></div>
											</div>
										</div>
									</div>
								</div>
									...
									<div class="col-md-12">
										<div class="col-md-6">
											...
											<h4>İş barədə məlumat</h4>
												<p>{{$vac->vac_job_description}}</p>
										</div>
										<div class="col-md-6">
											<h4>Namizədə tələblər</h4>
												<p>{{$vac->vac_requirements}}</p>
										</div>
									</div>
							</div>
